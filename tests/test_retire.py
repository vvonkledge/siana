...
class TheExternalAnchorRule(Retire):
    ...
    def test_a_base_this_repository_no_longer_has_is_not_an_anchor(self):
        ...
        qa = self.qa_of()
        self.tasks("done", "qa-make-thing", "--reason", "judged")
        # git refuses to delete a branch a worktree still holds, so the ship tree
        # goes first. What is under test is a base the queue names and git cannot
        # find, not how it came to be missing.
        self.git("worktree", "remove", ship)
